...
def api_notificacoes():
    ...
    # Conectar ao banco usando a função existente
...
def marcar_notificacao_lida():
    ...
    cursor.execute(
    "UPDATE notificacoes SET lida = 1 WHERE id = %s AND candidato_id = %s",
...
@notifications_bp.route("/api/notificacoes/marcar-todas-lidas", methods=["POST"])
def marcar_todas_notificacoes_lidas():
    ...
    cursor.execute(
        "UPDATE notificacoes SET lida = 1 WHERE candidato_id = %s ", (session["candidato_id"],)
        )
...
def apagar_notificacao(notificacao_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM notificacoes WHERE id = %s AND candidato_id = %s",
        (notificacao_id, session["candidato_id"])
    )
    conn.commit()
    conn.close()
...
def marcar_notificacao_individual_lida(notificacao_id):
    ...
    cursor.execute(
        "UPDATE notificacoes SET lida = 1 WHERE id = %s AND candidato_id = %s ", (notificacao_id, session["candidato_id"]))
...
def criar_notificacoes_demo():
    ...
    for mensagem, tipo in notificacoes_demo:
        cursor.execute(
            "INSERT INTO notificacoes (candidato_id, mensagem, tipo, lida, data_envio) VALUES (%s, %s, %s, 0, datetime('now'))", (session["candidato_id"], mensagem, tipo))
...
def apagar_todas_notificacoes():
    ...
    cursor.execute(
        "DELETE FROM notificacoes WHERE candidato_id = %s", (session["candidato_id"], ))
