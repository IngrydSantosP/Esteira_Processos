...
def processar_curriculo(resumo_profissional):
    ...
    for padrao in padroes_experiencia:
        match = re.search(padrao, resumo_profissional.lower(), re.DOTALL)
        if match:
...
def calcular_score_local(resumo_profissional, requisitos_vaga, pretensao_salarial, salario_oferecido):
    ...
    resumo_profissional_lower = resumo_profissional.lower()
    
    requisitos_encontrados = 0
    for requisito in requisitos_lista:
        if requisito in resumo_profissional_lower:
...
def calcular_score_huggingface(resumo_profissional, requisitos_vaga, pretensao_salarial, salario_oferecido):
    ...
    try:
        ...
        # Calcular embeddings
        embedding_curriculo = modelo.encode([resumo_profissional])
        ...
        return calcular_score_local(resumo_profissional, requisitos_vaga, pretensao_salarial, salario_oferecido)

def calcular_score_matching(resumo_profissional, requisitos_vaga, pretensao_salarial, salario_oferecido):
    """Calcula score baseado no modo de IA configurado"""
    if MODO_IA == 'huggingface':
        return calcular_score_huggingface(resumo_profissional, requisitos_vaga, pretensao_salarial, salario_oferecido)
    else:
        return calcular_score_local(resumo_profissional, requisitos_vaga, pretensao_salarial, salario_oferecido)
...
def dashboard_candidato():
    ...
    cursor.execute('SELECT resumo_profissional FROM candidatos WHERE id = ?', (session['candidato_id'],))
    candidato = cursor.fetchone()
    ...
    cursor.execute('SELECT pretensao_salarial, resumo_profissional FROM candidatos WHERE id = ?', (session['candidato_id'],))
...
def upload_curriculo():
    ...
    if request.method == 'POST':
        ...
        if arquivo and arquivo_permitido(arquivo.filename):
            ...
            resumo_profissional = extrair_texto_pdf(caminho_arquivo)
            
            if resumo_profissional:
                # Processar currículo
                experiencia, competencias, resumo_profissional = processar_curriculo(resumo_profissional)
                ...
                cursor.execute('''
                    UPDATE candidatos 
                    SET resumo_profissional = ?, experiencia = ?, competencias = ?, resumo_profissional = ?
                    WHERE id = ?
                ''', (resumo_profissional, experiencia, competencias, resumo_profissional, session['candidato_id']))
...
def candidatar(vaga_id):
    ...
    cursor.execute('''
        SELECT pretensao_salarial, resumo_profissional 
        FROM candidatos 
        WHERE id = ?
